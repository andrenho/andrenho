...
To test the capability of changing the rules during the execution of eProfile, a new simulation was run with $K=0.6$ until the 7th semester. Starting in this semester, LOCAL registered the rule changing to $K=0.5$, and eProfile started to consider this rule (in addition to the previous rules) in the student profile inference. 

Figure~\ref{fig:result2} shows the result of the second experiment. A sudden increase in the rate of recommended subjects can be seen from the 7th semester onward, which is the result of the modification of the rules during the execution of the simulation. The dotted line shows how would the results continue if the new rule hadn't been applied.

\begin{figure}[!t]
  \centering\includegraphics[width=0.7\textwidth,angle=-90]{imagens/temp2.eps}
