...
Foi desenvolvido um protótipo para avaliar o modelo, o que foi feito através de uma simulação (Capítulo~\ref{ch:implementacao}) de um ambiente de aprendizagem ubíqua, onde dois softwares (LOCAL e UniManager) foram modificados para integrarem-se ao eProfile. As trilhas geradas por ambos os aplicativos foi utilizada pelo eProfile para gerar perfis e realimentar estas aplicações com os perfis.

Foi desenvolvido um cenário onde foram simuladas as ações de alunos dentro de um contexto de educação ubíqua. As decisões tomadas pelos alunos referente a qual assunto estudar foram utilizadas para geração dos perfis. Os aplicativos utilizaram estes perfis para recomendar outros assuntos relacionados aos alunos. Foi verificado que a integração do eProfile a estes dois aplicativos fez com que eles pudessem recomendar um número cada vez maior de assuntos, de acordo com a passagem do tempo.
