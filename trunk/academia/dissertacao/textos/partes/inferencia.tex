...
Para que um cliente possa utilizar os serviços oferecidos pelo eProfile, é necessário que o cliente envie ao eProfile a ontologia de perfil que ele irá utilizar, extendendo a ontologia do eProfile. A ontologia é enviada quando o cliente faz seu primeiro acesso ao eProfile, e pode ser atualizada posteriormente. Ela deve ser enviada em formato OWL utilizando a sintaxe RDF/XML\cite{rdf}.
...
O cliente pode, a qualquer momento, criar, editar ou excluir regras para a inferência das informações de sua trilha, de modo a gerar informações úteis dentro da ontologia de perfil.
...
\begin{description}
  ...
  \item[Tipo de regra:] se é uma regra em OWL ou SPARQL;
...
\begin{description}
  ...
  \item[Expressão:] uma informação que precisa ser calculada. Neste caso, o próprio cliente passa a expressão do cálculo. Calcular o percentual de interesse de um usuário por um determinado assunto, por exemplo, seria uma informação deste tipo.
...
\subsubsection{Motor de Inferência}

Tanto as consultas via OWL quanto as consultas via SPARQL são executadas através de um motor de inferências externo. Foram estudados alguns raciocinadores que oferecem suporte a estas operações, entre os quais estão:
