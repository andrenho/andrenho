...
% DAML+OIL
O \textbf{DAML+OIL} é uma linguagem de representação de conhecimento baseada nas linguagens DAML e OIL. O DAML (\emph{DARPA Agent Markup Language}) foi desenvolvido pelo Centro de Pesquisas Avançadas da Defesa Americana e é uma linguagem de marcação de agentes baseada no RDF. Já o OIL (\emph{Ontology Inference Layer}) é uma linguagem com os mesmos objetivos, criada por um grupo de universidades européias. Ambos os esforços foram unidos na criação do DAML+OIL, que por sua vez deu origem ao OWL \cite{daconta03}.
...
% Escolhido - OWL
Devido à expressividade e ao suporte de ferramentas externas, o OWL foi escolhido como a linguagem de ontologias utilizada pelo eProfile para modelagem das ontologia de perfis e de trilhas.
...
Assim sendo, para a avaliação desta proposta será realizada uma abordagem sob duas perspectivas: {\bf funcionalidade} e {\bf cenário}. Na primeira, a avaliação se dará sobre as funcionalidades propostas, isto é, se o protótipo é funcional e atende o modelo proposto no Capítulo~\ref{ch:modelo}. Na segunda, serão desenvolvidos dois protótipos de aplicação (representando duas aplicações diferentes), que seguirão roteiros definidos em um conjunto de cenários. Será, então, verificado se o eProfile conseguiu cumprir as premissas descritas no cenário.

Para o teste de funcionalidades, serão verificadas as premissas descritas abaixo. Caso seja necessário, serão desenvolvidos programas capazes de realizar os seguintes testes:
...
Para a realização do teste de cenário, serão desenvolvidos de dois a três cenários que simularão atividades realizadas por múltiplas aplicações e múltiplas entidades, onde o cenário terá um resultado pré-definido, contra o qual será verificado o resultado obtido pelo protótipo do eProfile.
