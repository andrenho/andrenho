...
Foram estudados os seguintes raciocinadores que oferecem suporte a estas operações:
...
O trabalho apresenta um modelo que auxilia aplicações a montarem seu perfil de entidade baseado em uma trilha. O eProfile é uma proposta de infra-estrutura e, portanto, sua avaliação é problemática, uma vez que não é visível para o usuário final. Segundo \citetexto{edwards03}, somente é possível avaliar as funcionalidades de um framework construindo aplicações que as utilizem e então avaliá-las, de modo a obter uma avaliação indireta do framework.

Assim sendo, para a avaliação desta proposta será realizada uma abordagem sob duas perspectivas: {\bf funcionalidade} e {\bf cenário}. Na primeira, a avaliação se dará sobre as funcionalidades propostas, isto é, se protótipo é funcional e atende o modelo proposto no Capítulo~\ref{ch:modelo}. Na segunda, será desenvolvido dois protótipos de aplicação, que seguirão um roteiro definido em um cenário. Será, então, verificado se o eProfile conseguiu cumprir as premissas descritas no cenário.

Para o teste de funcionalidades, serão verificadas as premissas descritas abaixo. Caso seja necessário, serão desenvolvidos programas capazes de realizar os testes.
...
\item o eProfile é capaz de utilizar as regras para realizar a inferência das trilhas de uma entidade, e montar um perfil de acordo com o modelo de entidade;
\item o eProfile é capaz de realizar todos os testes acima para múltiplos gerenciadores de trilha, múltiplas aplicações e múltiplas entidades.

\end{itemize}

Para a realização do teste de cenário, serão desenvolvidos de dois a três cenários que simularão algumas atividades realizadas por múltiplas aplicações e múltiplas entidades, onde o cenário terá um resultado pré-definido, contra o qual será verificado o resultado obtido pelo protótipo do eProfile.
