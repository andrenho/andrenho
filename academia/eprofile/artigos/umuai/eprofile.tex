...
eProfile prototype was developed in Python \citep{python}. Python is a generic, open source, object-oriented, high-level language, that supports multiple architectures. 

eProfile class diagram is represented in Figure~\ref{fig:classes}. The Communicator, Configurator, Converter and Reasoner classes implement the agents with the same names described in Section~\ref{sec:arquitetura}, and all of them inherit from class Agent, where the commong agent tasks are implemented, such as setup, interagent communication and thread control.
...
The scenario objective was to evaluate the integration between eProfile, LOCAL and UniManager through a four year undergraduation course, which made a real-life scenario impratical. A simulation was thus created to validate the model. Two programs were developed:
...
Figure~\ref{fig:result1} shows the results of the simulation. On this simulation, a group of 40 students goes through a complete under-graduation course. The course takes more than 16 semesters because, due to limited number of openings in the disciplines, not all students take all disciplines available every semester. The simulator registers the percentage of the debates and papers subjects that were suggested by LOCAL, based on the profile generated by eProfile that, in turn, was inferred from previous student choices. In the first semester, the system was able to suggest only 2\% of the time, but as the students made more choices, the system was able to suggest more subjects, until the level of recommendation stabilized above 90\% in the 13th semester onward.

Multiple executions of the simulation outputted similar results.
...
\begin{itemize}
  ...
  \item Uses trails: defines if the trails are used when building the profile. In the table, ``Partial'' means the model uses some type of historical information to form the profile, but doesn't record the history of visited contexts and the actions done within these context;
...
Furthermore, while all the related works present updates of the user or entity profile according to external events, eProfile contributes the idea of dynamic models and rules, allowing applications to update the entity model or the profile inference rules during execution, simplifying the application process of adaptation to its users or entities.
...
The semantic interoperability of the model opens the possibility that one application can use information generated by a different application to create the profile of its entities. This is demonstrated in the scenario where UniManager uses information generated by LOCAL (the optional disciplines chosen by the students) as part of the data that is used to create the profiles used by UniManager. This information is later used to suggest optional disciplines to the students.

The scenario shows that is viable to integrate multiple systems with eProfile, enriching the profile generation of applications and allowing them to adapt themselves to their users. The scenario demonstrate the main contributions of the model.
