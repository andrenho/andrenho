...
Communication between these three applications (UniManager, LOCAL and eProfile) is done through REST web services \citep{fielding00}.
...
Figure~\ref{fig:result1} shows the results of the simulation. On this simulation, a group of 50 students goes through a complete undergraduation course. The course takes more than 16 semesters because, due to limited number of openings in the disciplines, not all students take all disciplines available every semester. The simulator registers the percentage of the debates and papers subjects whose subjects were be suggested by the system, based on previous student choices. In the first semester, the system was able to suggest only 2\% of the time, but as the students made more choices, the system was able to suggest more subjects, until the level of recommendation stabilized above 90\% in the 13th semester onwards.
...
To test the capability of changing the rules during the execution of eProfile, a new experiment was made. On this test, the execution was the same as the previous test until the 8th semester. On this semester, the rule for generating the listing of subjects related to the subject that is being debated or studied (as demonstrated in Figure~\ref{fig:regra1}) was changed: in addition to consider subjects related to the subject in point, the subjects chosen by the 5 most similar students to each student were also used as recommendation. In this context, the most similiar student is the one who made more choices equals to another student.

Figure~\ref{fig:result2} shows the result of the second experiment. Comparing Figure~\ref{fig:result1} and Figure~\ref{fig:result2}, it can be seen a sudden increase in the rate of recommended subjects from the 8th semester onwards, which is the result of the modification of the rules during the execution of the simulation.
...
This article has proposed a model to support the management of profiles in ubiquitous environments, allowing entities trails inference through definition of rules by applications. Although eProfile was integrated with LOCAL and UniManager in the scenario, its proposal is generic enough to be applied to any other systems that use profiles from improvement of personalization. The use of automatic profiles allows ubiquitous systems to be more efficient in understanding user intent, because this means this type of system can use more precise, up-to-date and broad information in a way that caters to the application specific need.
