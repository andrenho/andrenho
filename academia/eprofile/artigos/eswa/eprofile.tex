...
\title{A Model for Profile Management Applied to Context-Aware U-Learning Environments}
...
\address[UNISINOS]{Universidade do Vale do Rio dos Sinos (UNISINOS) \\
	           Programa de Pós-Graduação em Computação Aplicada (PIPCA) \\
                   Av. Unisinos, 950, São Leopoldo, Brazil}
\address[FEEVALE]{Universidade Feevale\\
...
Ubiquitous computing has been used successfully in several different environments and contexts, such as commerce \citep{franco11}, COMPLETAR. One such environment is ubiquitous learning, or U-Learning. Ubiquitous learning can be understood as a set of educational processes that occur anywhere, at any time and with any device, continuously, contextualized and integrated into the daily lives of apprentices \citep{barbosa11}. It is important to notice that U-Learning is more than simply ``learning with u-computing technology'' \citep{hwang07}, but is composed by a mobile environment that takes into consideration the learner's individual information of various sources in context, and adapts the instructional content to meet the functions of different mobile devies \citep{hwang08}.
...
In this setting we are proposing eProfile, a model for the automatic creation of profiles by inference of entity trails.
...
This paper is organized as follows. In Section~\ref{ch:relacionados}, we present a review of recent research on this subject. Section~\ref{ch:modelo} proposes the eProfile model. Section~\ref{ch:implementacao} discusses implementation aspects. Section~\ref{ch:cenario} presents an experiment validating the model and highlighting eProfile contributions. Finally, a Section~\ref{sec:conclusao} presents the concluding remarks.
...
\citet{wu12} presented a context-aware ubiquitous learning strategy using a cognitive apprenticeship approach applied to the standard operating process of a physical assessment nusring course the includes collecting patient' life signs and other information, where the students used a RFID-based mobile system that helped them on their learning. Experiment showed that the students' skill level improved more using a ubiquitous learning system than the traditional approach.
...
The evaluation scenario takes place in a simulated university called XXXXXX. This university has 20 different courses, each one having 40 disciplines. Some disciplines are shared between different courses. Each discipline has 3 to 5 different subjects. A student is required to master all subjects to pass a discipline, and master all subjects in all disciplines in order to graduate from the course. Mastery of a subject is proven through approval in tests and excercises.

All tests and exercises taken by the students are registered in an administrative software called UniManager. Based on a student records, UniManager register the proficiency level of the student. This information is used by the University to determine if the student can attend a discipline. UniManager also is used to control enrollment of students in disciplines.

Figure \ref{fig:unimap} presents the map of the university. Rooms depicted with dots are classrooms, where the students take classes. Rooms depicted with lines are convivence rooms. These might be study rooms, bars, restaurants, or any other rooms where the students go to rest and socialize. Students will go to these rooms anytime they are in the University but not taking classes.

Since most students remain idle during the time they are in these spaces, this situation provides an opportunity to put them in touch. Therefore, an integration between UniManager, eProfile and LOCAL was developed, in order to put in contact students that can help each other. Figure~\ref{fig:datacycle} explains the data flow.

\begin{figure}[!t]
  \centering\includegraphics[width=0.7\textwidth]{imagens/datacycle.png}
  \caption{Scenario Data Cycle}
  \label{fig:datacycle}
\end{figure}

As explained above, UniManager calculates the proficience of students in determinated subjects based on their evaluations. A student can be considered ``non-proficient'' or ``proficient''. In the moment that the student becomes proficient in a subject, this information is registered in a trail manager by UniManager (arrow ``a''). For this simulation, a very simple trail manager was created, that will simply store trail events in a database.

eProfile, following rules determined by UniManager, will create or update a student's profile with his proficience information, the disciplines that he's enrolled, 

% TODO - explain trail manager

\subsection{eProfile/LOCAL integration}


eProfile's purpose is to provide profile management to other applications. In order to evaluate this model, we integrated eProfile to a ubiquitous learning system called LOCAL (Location and Context-Aware Learning). LOCAL uses location information and context as an aid to teaching and learning process. A location system monitors the mobility of apprentices and, based on their physical position, explores educational opportunities \citep{barbosa11}.
