...
Nesta seção é mostrado o exemplo de funcionamento do modelo eProfile, detalhado nas seções anteriores. Este exemplo demonstra um curso de programação em linguagens orientadas a objeto, na qual os estudantes podem escolher quais assuntos específicos dentro da matérias eles vão estudar.
...
Neste exemplo, aplicativo móvel distribui os objetos de aprendizagem de forma contextualizada. O aplicativo registra em um gerenciador de trilhas cada vez que um módulo é iniciado ou concluído. O aplicativo móvel, o gerenciador de trilhas e o eProfile conectam-se entre si de acordo com a Figura~\ref{fig:modelobasico} (Seção~\ref{sec:modelo}).

Cada evento (início ou conclusão de um objeto de aprendizagem) é registrado contendo as seguintes informações: (i) entidade que realizou o evento, (ii) objeto de aprendizagem, (iii) tipo de evento (início ou conclusão), (iii) data e hora e (iv) se é a primeira vez que o estudante realiza este tipo de evento. Neste exemplo, os eventos descritos na Tabela~\ref{tab:eventos} foram registrados pelo aplicativo no gerenciador de trilhas durante um período de um dia.
